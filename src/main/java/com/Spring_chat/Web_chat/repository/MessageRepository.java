...
public interface MessageRepository extends JpaRepository<Message, Long> {
    @Query(value = """
        ...
        WHERE m.conversation_id = :convId
          AND (:beforeId IS NULL OR (
              m.created_at < (SELECT m2.created_at FROM messages m2 WHERE m2.id = :beforeId)
              OR (m.created_at = (SELECT m3.created_at FROM messages m3 WHERE m3.id = :beforeId) AND m.id < :beforeId)
          ))
